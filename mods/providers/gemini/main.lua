-- Gemini 原生协议插件 (generativelanguage.googleapis.com)

function onLoad()
    alice.service.register("ai.protocol.gemini", function(method, args_json)
    ...
    alice.log.info("Gemini 协议插件已注册")
end

function messages_to_contents(messages)
    local contents = {}
...
function parse_gemini_response(resp_raw, model)
    if not resp_raw then return alice.json.encode({ error = "HTTP 请求失败" }) end
    local resp = alice.json.decode(resp_raw)
    if not resp or resp.status ~= 200 then
        return alice.json.encode({ error = "Gemini 错误 " .. tostring(resp and resp.status or "nil") })
    end
...
function do_chat(args_json)
    ...
    local base_url = cfg.base_url or "https://generativelanguage.googleapis.com"

    local url = base_url .. "/v1beta/models/" .. model .. ":generateContent?key=" .. api_key
    local contents = messages_to_contents(args.messages or {})
    ...
    return parse_gemini_response(resp_raw, model)
end
...
function do_stream_chat(args_json)
    ...
    local url = base_url .. "/v1beta/models/" .. model .. ":streamGenerateContent?alt=sse&key=" .. api_key
    local contents = messages_to_contents(args.messages or {})
...
function onUnload() alice.log.info("Gemini 协议插件已卸载") end
